...
async function handleGemini(system, messages, max_tokens) {
  ...
  console.log(
    `[Gemini] finishReason=${data?.candidates?.[0]?.finishReason} tokens(prompt/thoughts/candidates)=${data?.usageMetadata?.promptTokenCount}/${data?.usageMetadata?.thoughtsTokenCount}/${data?.usageMetadata?.candidatesTokenCount}`
  );
